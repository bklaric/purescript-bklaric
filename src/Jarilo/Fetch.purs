...
import JavaScript.Error (message)
import JavaScript.Web.Fetch.Async (text)
...
instance (WriteForeign realBody, ReadForeign realBody) => FetchBody (JsonBody realBody) realBody where
    ...
    fetchResponseBody _ response =
        (response # text # lmap message)
        >>= (JsonAsync.readJSON >>> lmap show)
...
-- | Which of the three values `RequestInit.credentials` accepts to send. The
-- | spec types it as a fixed set of strings; this is the same set as a sum, so
-- | a caller writes `Just Include` rather than a cast string literal.
data Credentials = Omit | SameOrigin | Include

type CredentialsLit =
    StringLit "omit" |+| StringLit "same-origin" |+| StringLit "include"

credentialsLit :: Credentials -> CredentialsLit
credentialsLit Omit = asOneOf (stringLit :: StringLit "omit")
credentialsLit SameOrigin = asOneOf (stringLit :: StringLit "same-origin")
credentialsLit Include = asOneOf (stringLit :: StringLit "include")
...
instance
    ...
    fetch _ pathParams queryParams realBody { origin, pathPrefix, credentials } = let
        ...
        options :: RequestInit
        options =
            { body: maybe (asOneOf undefined) asOneOf
                (fetchRequestBody (Proxy :: _ requestBody) realBody)
            , credentials: maybe (asOneOf undefined) (credentialsLit >>> asOneOf)
                credentials
