...
-- The body readers below shadow their promise-returning namesakes, so only the
-- type and the pure accessors are re-exported.
import JavaScript.Web.Fetch.Response (Response, ok, status, statusText) as Response
import JavaScript.Web.Fetch.Response (Response)
import JavaScript.Web.Fetch.Response as Res
...
text response = Res.text response # promiseToAsync

json :: Response -> Async Error Foreign
json response = Res.json response # promiseToAsync

blob :: Response -> Async Error Blob
blob response = Res.blob response # promiseToAsync

arrayBuffer :: Response -> Async Error ArrayBuffer
arrayBuffer response = Res.arrayBuffer response # promiseToAsync
